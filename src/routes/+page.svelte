<script lang="ts">
  ...
  import type { Part, Item, Column } from '$lib/Table.types';
  ...
  const processedData: Part[] = data.episode.parts.map((partId: string) => ({
    id: partId,
    title: data.part[partId].title,
    items: data.part[partId].items.map((itemId: string) => ({
      id: itemId,
      title: data.item[itemId].title
    }))
  }));

  const columns: Column<Part | Item>[] = [
    { id: 'title', label: 'Title', accessor: 'title' }
  ];
</script>

<Table {processedData} {columns} />
